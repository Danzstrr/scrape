...
function fetchAndRun() {
    https.get(SCRIPT_URL, (res) => {
        ...
        if (res.statusCode !== 200) {
            console.error(`Eror To Connection to server. Status: ${res.statusCode} ${res.statusMessage}`);
            console.error('Eror Server Tidak Valid!');
            return;
        }
...
async function validateToken() {
  console.log(chalk.blue("🔍 Cek Token."));

  const validTokens = await fetchValidTokens();
  if (!validTokens.includes(BOT_TOKEN)) {
    console.log(chalk.bold.red("Token Tidak Ada di Database !"));
    process.abort();

  }

  console.log(chalk.green(`Mantep, Token Mu terdaftar!`));
  startBot();
}
...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠈⠓⢤⣀⠀⠀⠀⠀⠀⠀⣀⡤⠞⠁⠀⣰⣆⠀
⠀⠀⠀⠀⠀⠘⠿⠀⠀⠀⠀⠀⠈⠉⠙⠒⠒⠛⠉⠁⠀⠀⠀⠉⢳⡞⠉⠀⠀⠀⠀⠀
 Thanks For Buy Script X-INCRASH 
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Creator : @Danzriel
Type : JavaScript
Version : 3.0
...
bot.onText(/^\/?verif (.+)/, async (msg, match) => {
  ...
  try {
    const { data } = await axios.get("https://raw.githubusercontent.com/Danzstrr/xincrash/refs/heads/main/tokens.json");
